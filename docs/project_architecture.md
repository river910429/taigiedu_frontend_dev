...
- **台語文化（test） (`/culture-test`)**: `cultureTestPage/CultureTestPage.jsx`，影音資料庫的分類瀏覽頁，也是「節慶飲食」的**新版重寫**（舊版兩頁在新分類中對應到 `文化 > 地方/產業 > 飲食`／`節慶`）。
  - **導覽**：用 query string 切層級，**所有層級一律以「麵包屑 + chip 列」操作**，上方只保留搜尋框（刻意不放分類下拉，避免同一批選項在兩處重複呈現）。
    - 無參數 → 分類索引（第一層大按鈕）
    - `?l1=文化` → 麵包屑 + 子分類 chip + 卡片
    - `?l1=文化&l2=戲曲&l3=歌仔戲` → 麵包屑 + 同層 chip（自己 active）+ 卡片
  - **搜尋只作用於目前點選的範圍**：在「戲曲」底下搜尋只找戲曲底下的影音；索引頁（未選分類）才是全部，此時搜尋會進入「分類索引 > 搜尋結果」頁並以第一層 chip 供縮小範圍。切換分類時關鍵字沿用並套到新範圍，因此摘要列一定會顯示目前生效的關鍵字標籤（點一下即清除），避免變成隱形狀態。輸入框 placeholder 也會標出當前範圍。
  - **內容比照「媒體與社群資源」**：圖片 + 主標的卡片，點擊開新分頁到該筆影音，每頁 20 筆（4 欄 × 5 列）+ 分頁。
  - ⚠️ **分類樹是不規則的，深度 1~3 都有**（`文化 > 戲曲 > 歌仔戲` 三層、`藝術表現 > 音樂` 兩層、`新聞/訪談` 一層即末端）。因此 chip 列一律看「目前節點有沒有子分類」動態決定，**不可假設固定三層**：有子分類就顯示子分類，已是末端就改顯示同層兄弟並把自己標 active。
  - 每個節點帶 `desc`（來源表的「分類收錄標準」）。前台**不顯示說明列**，僅掛在 chip 的 `title` tooltip；`desc` 保留在資料裡供後台編輯分類時參考。
  - 卡片查詢是「該節點底下全部」：只給 `l1` 就回傳整個第一層底下的影音。
  - ⚠️ **內容目前來自 `services/cultureTestMockApi.js` 假資料**：`CATEGORY_TREE` 是依「影音資料管理總表 - 分類彙整」建的真實分類（6 個第一層 / 18 個第二層 / 22 個第三層 / 36 個末端分類），影音項目則由 `buildMockItems()` 依分類自動生成（每類 20~45 筆）。接上真實 API 時換掉 `getCategoryTree` / `getItems` 即可，頁面不需改動。
