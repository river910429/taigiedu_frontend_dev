...
const SortableFoodRow = ({ item, handleEditClick, handleDeleteClick, statusFilter, handlePlayAudio, isPlaying }) => {
...
const AdminFoodPage = () => {
  ...
  const activeItem = useMemo(() => foodList.find(i => i.id === activeId), [activeId, foodList]);
  ...
  const fetchFood = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setFoodList([]); setError(e.message);
  ...
  useEffect(() => {
    if (allFood.length > 0 || !isLoading) {
      let filtered = allFood.filter(i => i.status === statusFilter);
      if (sortField) {
        filtered.sort((a,b) => {
  ...
  }, [allFood, statusFilter, isLoading, sortField, sortDirection]);
  ...
  return (
    <div className="admin-test-page p-4">
      ...
      {isLoading ? (
        ...
        <div className="news-loading"><p>載入失敗，請重新整理頁面</p><button onClick={fetchFood} className="btn btn-primary mt-2">重新載入</button></div>
      ) : foodList.length === 0 ? (
            ...
            <table className="table admin-data-table">
              <thead>
                <tr>
                  ...
                  <th className="admin-table-header" onClick={() => handleSort('twName')} style={{cursor:'pointer'}}>名稱(台羅) <span className="sort-arrow">{sortField === 'twName' ? (sortDirection === 'asc' ? '↑' : '↓') : '↓'}</span></th>
              ...
              <SortableContext items={foodList.map(i => i.id)} strategy={verticalListSortingStrategy}>
                <tbody>
                  {foodList.map(item => (
                    <SortableFoodRow key={item.id} item={item} handleEditClick={handleEditClick} handleDeleteClick={handleDeleteClick} statusFilter={statusFilter} handlePlayAudio={handlePlayAudio} isPlaying={playingId === item.id} />
                  ))}
                </tbody>
              </SortableContext>
            </table>
                ...
                <table className="table admin-data-table drag-overlay-table">
                  <tbody>
                    <tr className="dragging-overlay-row">
                      ...
                      <td>{activeItem.audioUrl ? (<div className="audio-cell"><button type="button" className="audio-play-btn" disabled><img src={speakerIcon} alt="播放" className="audio-icon" /></button></div>) : '無'}</td>
