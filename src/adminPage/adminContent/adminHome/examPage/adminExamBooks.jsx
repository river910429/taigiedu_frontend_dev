...
const AdminExamBooks = () => {
  ...
  useEffect(() => {
    filterAndSortItems();
  }, [books, statusFilter, sortField, sortDirection]);
  ...
  const filterAndSortItems = () => {
    let filtered = books.filter(item => item.status === statusFilter);
        ...
        <table className="table table-hover admin-data-table">
          <thead>
            <tr>
              ...
              <th className="admin-table-header" onClick={() => handleSort('bookTitle')}>
                書名 <span className="sort-arrow">{getSortArrow('bookTitle')}</span>
              </th>
              <th className="admin-table-header" onClick={() => handleSort('author')}>
                作者 <span className="sort-arrow">{getSortArrow('author')}</span>
              </th>
              <th className="admin-table-header" onClick={() => handleSort('publisher')}>
                出版社 <span className="sort-arrow">{getSortArrow('publisher')}</span>
              </th>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content admin-modal-content">
            <div className="modal-header admin-modal-header">
              <h5 className="modal-title">{isEditing ? '編輯書籍' : '新增書籍'}</h5>
